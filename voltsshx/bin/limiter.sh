...
droppids() {
  ...
  loginsuccess='Password auth succeeded'
  pids=$(ps ax | grep 'dropbear' | grep " $port_dropbear" | awk -F " " '{print $1}')

  msg -bar
  print_center -ama "${a8:-SUCCESSFUL DROPBEAR LOGINS}"
  msg -bar

  for pid in $pids; do
    pidlogs=$(grep $pid $log | grep "$loginsuccess" | awk -F" " '{print $3}')
    i=0
    for pidend in $pidlogs; do
      let i=i+1
    done
    if [ $pidend ]; then
      login=$(grep $pid $log | grep "$pidend" | grep "$loginsuccess")
      PID=$pid
...
sshmonitor() {
  ...
  timer=$(cat /etc/limit)
  [[ -z ${timer} ]] && timer="3"
  at now +${timer} minutes <<<"/etc/voltsshx/bin/limiter.sh" &>/dev/null

  [[ -z $(cat "/var/spool/cron/crontabs/root" | grep "limiter.sh") ]] && print_center -ama "@reboot root /etc/voltsshx/bin/limiter.sh" >>/var/spool/cron/crontabs/root
}
